...
VMUINT32 mic_read(VMUINT8* buf, VMUINT32 buf_size, VMUINT32* readed)
{
    ...
    if (!api_ready || !buf || buf_size == 0)
        return 0;

    VMUINT32 total_bytes_read = 0;
    VMUINT32 bytes_needed = buf_size;

    for (int i = 0; i < 2 && bytes_needed > 0; ++i)
    {
        VMUINT16* mic_buf = 0;
        VMUINT32 mic_buf_len_words = 0;
        ...
        VMUINT32 available_bytes =
            mic_buf_len_words * sizeof(VMUINT16);

        VMUINT32 bytes_to_copy =
